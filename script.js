...
function setNextFruit() {
    ...
    fruitCircle.style.position = 'absolute';
    fruitCircle.style.left = '50%';
    fruitCircle.style.top = '50%';
    fruitCircle.style.transform = 'translate(-50%, -50%)';
    nextFruitPreview.appendChild(fruitCircle);
}

// 添加水果的函数
function addFruit(x) {
    if (gameOver || !nextFruit || currentActiveFruit) return;
    ...
    currentActiveFruit = fruit;

    // 延时清除 currentActiveFruit，等待水果下落
    setTimeout(() => {
        currentActiveFruit = null;
        setNextFruit(); // 设置下一个水果
    }, 500); // 假设0.5秒后水果稳定
}

// 监听鼠标点击事件来添加水果
gameContainer.addEventListener('click', function(event) {
    if (gameOver || currentActiveFruit) return; // 游戏结束或有水果正在下落时不能添加新水果
...
function startGame() {
    // 清除所有现有水果
    World.allBodies(world).filter(body => body.label === 'fruit').forEach(body => World.remove(world, body));
    ...
    Runner.run(runner, engine);
    Render.run(render);
    gameContainer.style.pointerEvents = 'auto'; // 启用点击事件
}
    ...
    for (let i = 0; i < bodies.length; i++) {
        ...
        if (body.label === 'fruit' && body.position.y < deathLine) {
            ...
            if (body.velocity.x < 0.1 && body.velocity.y < 0.1) {
                gameOver = true;
                saveScores(score); // Save scores before alerting
                updateScoreDisplay(); // Update display after saving
                alert('Game Over!');
                Runner.stop(runner);
                Render.stop(render);
                gameContainer.style.pointerEvents = 'none'; // 禁用点击事件
                break;
            }
        }
    }
